  ...
  <v-card dark class="dialog-map-info ff-wkw5 gc-selection">
    ...
    <v-card-text dark class="py-15-px">
      <div class="ff-wkw5 ma-1-pct">
        <div
          v-if="
            client.dialog_gc_selection_dataset.data.length >
            client.dialog_gc_selection_answers.length
          "
          class="flex-no-wrap justify-space-between"
        >
          <div
            v-for="(dd, idx) in client.dialog_gc_selection_dataset.data"
            :key="idx"
          >
            <div v-if="idx == client.dialog_gc_selection_answers.length">
              <h2 class="gc-selection-title">{{ dd.text }}</h2>
              <ul
                v-if="dd.options && dd.options.length > 0"
                class="gc-selection-options"
              >
                <li
                  v-for="op in dd.options"
                  :key="op.value.id"
                  class="gc-selection-li"
                  :class="{
                    enable:
                      typeof op.enable == 'function'
                        ? op.enable(client.dialog_gc_selection_answers[idx - 1])
                        : op.enable
                  }"
        ...
        <div class="btns">
          <v-btn
            v-if="client.dialog_gc_selection_answers.length > 0"
            @click="onClickBack"
            >返回</v-btn
          >
          <v-btn
...
export default Vue.extend({
  ...
  methods: {
    ...
    onClickDailogSelection: function (option, deep) {
      ...
      if (
        typeof option.enable == 'function'
          ? option.enable(nextAnswers[deep - 1])
          : option.enable
      ) {
    ...
    onClickDailogSumbit: function () {
      ...
      const inputs =
        this.client.dialog_gc_selection_dataset.data[deep].inputs || []
      const nextAnswers = this.client.dialog_gc_selection_answers.slice()
    ...
    makeSure: function (answers) {
      if (
        answers.length >= this.client.dialog_gc_selection_dataset.data.length
      ) {
